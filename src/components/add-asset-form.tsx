...
export default function AddAssetForm() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast({ title: "Asset Registered", description: `${assetData.name} has been added to the system.` });
      (event.target as HTMLFormElement).reset();
      setHealthScore(100);
    } catch (error) {
      toast({ variant: "destructive", title: "Error", description: "Integration failed. Check database connectivity." });
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <Card className="border-none shadow-xl ring-1 ring-border overflow-hidden">
      ...
      <CardHeader className="pb-8">
        ...
        <CardDescription>Onboard a new city component into the surveillance network.</CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="grid gap-6 md:grid-cols-2">
            <div className="space-y-3">
              ...
              <Input id="name" name="name" placeholder="e.g. Skyline Metro Bridge" required className="bg-muted/30 h-11" />
            </div>
            <div className="space-y-3">
          ...
          <div className="space-y-6 pt-4 bg-muted/20 p-6 rounded-xl border border-dashed">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-baseline gap-1">
                <span className="text-2xl font-black text-primary">{healthScore}</span>
